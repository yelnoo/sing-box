...
func (m *Manager) Start(stage adapter.StartStage) error {
	...
	case adapter.StartStateStart:
		...
		for _, provider := range providers {
			if startContexter, isStartContexter := provider.(interface {
				StartContext(context.Context, *adapter.HTTPStartContext) error
			}); isStartContexter {
				err := startContexter.StartContext(m.ctx, startContext)
				if err != nil {
